      ...
      if (!token) {
        ...
        if (oldToken == null) {
          ...
          if (oldToken.type == "new") {
            let newToken = await models_1.db.models.tokens.create({
              customerId: null,
              type: "register",
              target: email,
            });
      ...
      if (!email || !tokenType) {
        ...
        if (tokenType == "forgot") {
          ...
          if (customer == null) {
            ...
            await models_1.db.models.tokens.destroy({
              where: { target: email },
            });
            let token = models_1.db.models.tokens.build({
              customerId: customer.customerId,
              type: "forgot",
              target: email,
            });
            let emailContents = utility_1.Utility.buildMail(
              token.target,
              "パスワードリセット",
          ...
          let count = await models_1.db.models.customers.count({
            where: { email: email },
          });
          if (count > 0) {
    ...
    try {
      if (req.session.user) {
        req.session?.destroy((err) => {
          ...
          else res.clearCookie(process.env.SESS_NAME);
        });
      }
      let { token, password } = req.body;
      if (!token || !password) {
        throw new utility_1.DataConflictError(
          `invalid parameters ${JSON.stringify(req.body)}`
        );
      }
      transaction = await models_1.sequelize.transaction();
      let tokenDB = await models_1.db.models.tokens.findOne({
        where: { tokenId: token, type: "reset" },
      ...
      if (tokenDB == null || !tokenDB.target) {
        ...
        let customer = await models_1.db.models.customers.findOne({
          where: { email: tokenDB.target },
          transaction: transaction,
        });
        if (customer == null) {
          throw new utility_1.DataConflictError(
            `customer with email: ${tokenDB.target} does not exist`
          );
        }
      ...
      if (e instanceof utility_1.DataConflictError) {
        (0, utility_1.errLog)(
          `CustomerResetPassword DataConflictError ${e.message}`,
          "db"
        );
        res.sendStatus(constants_1.BAD_REQUEST);
      } else {
        (0, utility_1.errLog)(
          `CustomerResetPassword ${e.toString()} ${JSON.stringify(e)}`,
          "db"
        );
        res.sendStatus(constants_1.SYSTEM_ERROR);
      }
  ...
  static async CustomerSignup(req, res) {
    ...
    try {
      ...
      if (
        ...
        !gender ||
        !dateOfBirth ||
        q2inspectionCount === undefined ||
        q3inspectionPurpose === undefined ||
        !consent1 ||
        !consent2
      ) {
        throw new utility_1.DataConflictError(
      ...
      if (oldToken == null) {
        ...
        let emailCount = await models_1.db.models.customers.count({
          where: { email: oldToken.target },
          transaction: transaction,
        });
        if (emailCount > 0) {
          await transaction.rollback();
          ...
          let customer = await models_1.db.models.customers.create(
            {
              ...
              q2inspectionCount: q2inspectionCount,
              q3inspectionPurpose: q3inspectionPurpose,
              q4isVaccinated: q4isVaccinated,
              q5unvaccinatedReason: q5unvaccinatedReason,
              consent1: consent1,
              consent2: consent2,
            },
            { transaction: transaction }
      ...
      if (e instanceof utility_1.DataConflictError) {
        ...
        (0, utility_1.errLog)(
          `CustomerSignup ${JSON.stringify(e)} ${e.toString()}`,
          "db"
        );
        res.sendStatus(constants_1.SYSTEM_ERROR);
      }
    ...
    try {
      ...
      let customer = await models_1.db.models.customers.findOne({
        where: {
          ...
          pwhash: { [sequelize_1.Op.not]: null },
        },
      });
      if (customer == null) {
        return res.sendStatus(constants_1.PERMISSION_ERROR);
      }
      let isMatch = await utility_1.Utility.comparePassword(
        req.body.password,
        customer.pwhash
      );
      if (isMatch) {
        ...
        req.brute.reset(function () {
          res
            ...
            .send({ customerId: customer.customerId, role: "customer" });
        });
      } else {
        return res.sendStatus(constants_1.PERMISSION_ERROR);
      }
